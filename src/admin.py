...
class BotAdmin(commands.Cog):
    """
    Global administration commands for the bot. These are all owner-only and DM-only in the vast majority of cases.
    """
    ...
    async def leave_server(self, ctx: commands.Context, server_id: int) -> None:
        try:
            ...
            await ctx.send(f"Successfully left guild {self.bot.get_guild(server_id).name} (ID {server_id})")
